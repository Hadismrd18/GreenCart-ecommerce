...
function page() {
  ...
  useEffect(() => {
    ...
    setSellerId(sellerData[0]._id);
    console.log(sellerData[0]);
  }, [sellerData]);
  // save the orders related to that seller in redux
  useEffect(() => {
    if (!sellerId) {
      return;
    }
  ...
  const raw_orders = useSelector((state) => (sellerId ? state.order : []));
